...
if ($CategoryID == "") {
    ...
    $Query = "
                SELECT SI.StockItemID, SI.StockItemName, SI.MarketingComments, SI.IsChillerStock, TaxRate, RecommendedRetailPrice, ROUND(TaxRate * RecommendedRetailPrice / 100 + RecommendedRetailPrice,2) as SellPrice,
                QuantityOnHand,
                (SELECT ImagePath
...
if ($CategoryID !== "") {
$Query = "
            SELECT SI.StockItemID, SI.StockItemName, SI.MarketingComments, SI.IsChillerStock, TaxRate, RecommendedRetailPrice,
...
<div id="FilterFrame"><h2 class="FilterText"><i class="fas fa-filter"></i> Filteren </h2>
    <form action="" method="post">
        <input style="color:white;border-radius:15px;background-color:rgba(255,255,255,0);border: 2px solid white;width:80px;height:30px;position:fixed;margin-left:170px;margin-top:-38px;" type="submit" name="Aan/uit" value="On/Off"> <!--onclick="alert('Temperatuur wordt gemeten!')" -->
    </form>
    <form>
    ...
    <ul class="menu">
        ...
        <li><a href="contact.php">Contact pagina</a></li>
    ...
    if (isset($ReturnableResult) && count($ReturnableResult) > 0) {
        foreach ($ReturnableResult as $row) {
            ...
            <!-- einde coderegel 1 van User story: bekijken producten   -->
                ...
                if ($isChillerstock == 1){
                    $temperatuur = "SELECT MAX(Temperatuurmeting) 'Temp' FROM Temperatuurmeting";
                    $temperatuur2 = mysqli_query($databaseConnection, $temperatuur);
                    $temperatuur3 = mysqli_fetch_assoc($temperatuur2);
                    $temperatuur4 = $temperatuur3['Temp'];
                    print("<p style=\"position:absolute;margin-top:200px;margin-left:250px;font-family:Arial;\"> De huidige temperatuur is: ".$temperatuur4."°C</p>");
                }
                ...
                <div id="ProductFrame">
                    ...
                    if (isset($row['ImagePath'])) { ?>
                        <a class="ListItem" href='view.php?id=<?php print $row['StockItemID']; ?>'><div class="ImgFrame"
                            style="background-image: url('<?php print "Public/StockItemIMG/" . $row['ImagePath']; ?>'); background-size: 230px; background-repeat: no-repeat; background-position: center;"></div></a>
...
<!-- einde code deel 4 van User story: Zoeken producten  -->
        ...
        We hebben helaas geen resultaten gevonden voor jouw zoekopdracht, Pas je filterinstellingen aan, of zoek met een andere term.
        </h2>
        <?php
    }
    ?>
